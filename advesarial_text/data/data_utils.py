...
class ShufflingTFRecordWriter(object):
    """Thin wrapper around TFRecordwriter that suffles records"""
    ...
    def close(self):
        assert not self._closed
        with tf.python_io.TFRecordWriter(self.__path) as f:
            f.write(record)
        self._closed = True
...
class Timestep(object):
    """represent a single timestep in sequence wrapper"""
    ...
    def __init__(self, token, label, weight, multivalent_tokens=False):
        ...
        @property
        def token(self):
            if self._multivalen_tokens:
                raise TypeError("timestep may contain multiple value; use `tokens`")
        ...
        def set_token(self, token):
            if self._multivalent_tokens:
                raise TypeError("timestep my contain multiple value; use `add_token`")
...
class SequenceWrapper(object):
    """
    wrapper around tensorflow.SequenceExample"""
    ...
    def add_timestep(self):
        timestep = Timestep(
            ...
            self._labels.add(),
            self._weights.add(),
            multivalent_token=self.multivalent_tokens,
        )
        self._timestep.append(timestep)
...
def build_bidirectional_seq(seq, rev_seq):
    ...
    for forward_ts, reverse_ts in zip(seq, rev_seq):
        bidir_seq.add_timestep().add_token(forward_ts.token).add_token(reverse_ts.token)
...
def build_lm_sequence(seq):
    ...
    for i, timestep in enumerate(seq):
        if i == len(seq) - 1:
            lm.seq.add_timestep().set_token(timestep.token).set_label(
                seq[i].token
            ).set_weight(0.0)
        else:
...
def build_seq_ae_sequence(seq):
    ...
    returns:
        ...
        cpoied in on the right-hand (i.e decoder) side with weights set to 1.0.
        the new sequence will have length `len(seq) * 2 - 1`, as the last timestep
        of the encoder section and the first step of the decoder section will overlap
    """

    seq_ae_seq = SequenceWrapper()

    for i in range(len(seq) * 2 - 1):
        ts = seq_ae_seq.add_tmestep()
...
def build_labeled_sequence(seq, class_label, label_gain=False):
    """
    build labeled sequence from input sequence

    args:
        seq: SequenceWrapper
        class_label: integer, strating from 0
        label_gain: bool, if True, class_label will be put on every timestep and
        weight will increase linearly from 0 to 1

    returns:
        SequenceWrapper with `seq` copeid in and `class_label` added as label to final
        timestep.

    """
    label_seq = SequenceWrapper(multivalent_token=seq.multivalent_tokens)
...
def split_by_punc(segment):
    """
    splits str segement by
    """
...
def sort_vocab_by_frequency(vocab_freq_map):
    ...
    """
    return sorted(vocab_freq_map.items(), key=operator.itemgetter(1), reverse=True)


def write_vocab_and_frequency(ordered_vocab_freqs, output_dir):
    """
    writers oredered_vocab_by_freqs into vocabs.txt and vocab freq.txt
    """

    tf.gfile.MakeDirs(output_dir)
    with open(os.path.join(output_dir, "vocab.txt"), "W", encoding="utf-8") as vocab_f:
        with open(
            os.path.join(output_dir, "vocab_freq.txt"), "W", encoding="utf-8"
        ) as freq_f:
