...
flags.DEFINE_string(
    "imdb_input_dir",
    "",
    "The input directory containing the " "IMDB sentiment dataset.",
)
flags.DEFINE_integer(
    "imdb_validation_pos_start_id",
    10621,
    "File id of the " "first file in the pos sentiment validation set.",
)
flags.DEFINE_integer(
    "imdb_validation_neg_start_id",
    10625,
    "File id of the " "first file in the neg sentiment validation set.",
)
...
flags.DEFINE_string(
    "dbpedia_input_dir",
    "",
    "Path to DBpedia directory containing train.csv and " "test.csv.",
)
...
flags.DEFINE_string(
    "rcv1_input_dir",
    "",
    "Path to rcv1 directory containing train.csv, unlab.csv, " "and test.csv.",
)
...
def imdb_documents(dataset="train", include_unlabeled=False, include_validation=False):
    ...
    def check_is_validation(filename, class_label):
        ...
        is_neg_valid = (
            not class_label and file_idx >= FLAGS.imdb_validation_neg_start_id
        )
    ...
    for d, class_label in dirs:
        for filename in os.listdir(os.path.join(FLAGS.imdb_input_dir, d)):
            ...
            with open(
                os.path.join(FLAGS.imdb_input_dir, d, filename), encoding="utf-8"
            ) as imdb_f:
                content = imdb_f.read()
            yield Document(
                content=content,
...
def dbpedia_documents(
    dataset="train", include_unlabeled=False, include_validation=False
):
