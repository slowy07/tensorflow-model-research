...
class GraphsTest(tf.test.TestCase):
    ...
    def setUpClass(cls):
        ...
        seqs_bidir = [
            data.build_bidirectional_seq(seq, rev) for seq, rev in zip(seqs, seqs_rev)
        ]
        seqs_bidir_label = [
        ...
        for fname, seq_list in zip(filenames, seq_lists):
            with tf.python_io.TFRecordWriter(
                os.path.join(FLAGS.data_dir, fname)
            ) as writer:
                for seq in seq_list:
    ...
    def testATMethods(self):
        ...
        for method in at_methods:
            ...
            with tf.Graph().as_default():
                ...
                expected_num_vars = (
                    1 + 2 * FLAGS.rnn_num_layers + 2 * (FLAGS.cl_num_layers) + 2
                )
    ...
    def testBidirClassifier(self):
        ...
        for method in at_methods:
            ...
            with tf.Graph().as_default():
                ...
                expected_num_vars = (
                    1 + 2 * 2 * FLAGS.rnn_num_layers + 2 * (FLAGS.cl_num_layers) + 2
                )
                self.assertEqual(len(tf.trainable_variables()), expected_num_vars)
