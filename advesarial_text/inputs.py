...
class VatxtInput(object):
    ...
    def __init__(self, batch, state_name=None, tokens=None, num_states=0, eos_id=None):
        ...
        self._tokens = (
            ...
            else batch.sequences[data_utils.SequenceWrapper.F_TOKEN_ID]
        )
...
def _split_bidir_tokens(batch):
    ...
    forward, reverse = [tf.squeeze(t, axis=[2]) for t in tf.split(tokens, 2, axis=2)]
...
def _filenames_for_data_spec(phase, bidir, pretrain, use_seq2seq):
    ...
    data_specs = {
        ("train", True, True, False): (data_utils.TRAIN_LM, data_utils.TRAIN_REV_LM),
        ...
        ("test", True, True, False): (data_utils.TEST_LM, data_utils.TRAIN_REV_LM),
...
def _read_single_sequence_example(file_list, tokens_shape=None):
    ...
    ctx, sequence = tf.parse_single_sequence_example(
        serialized_record,
        sequence_features={
            data_utils.SequenceWrapper.F_TOKEN_ID: tf.FixedLenSequenceFeature(
...
def _read_and_batch(
    ...
    if bidir_input:
        rev_state_names = _get_tuple_state_names(
            num_layers, "{}_reverse".format(state_name)
        )
