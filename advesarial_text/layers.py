...
class Embedding(K.layers.Layer):
    ...
    def _normalize(self, emb):
        ...
        var = tf.reduce_sum(weights * tf.pow(emb - mean, 2.0), 0, keep_dims=True)
...
class SoftmaxLoss(K.layers.Layer):
    ...
    def __init__(
        self, vocab_size, num_candidate_samples=-1, vocab_freqs=None, **kwargs
    ):
        self.vocab_size = vocab_size
    ...
    def call(self, inputs):
        ...
        if self.num_candidate_samples > -1:
            ...
            lm_loss = tf.reshape(
                lm_loss, [int(x.get_shape()[0]), int(x.get_shape()[1])]
            )
        else:
...
def predictions(logits):
    ...
    with tf.name_scope("predictions"):
        ...
        if inner_dim == 1:
            pred = tf.cast(tf.greater(tf.squeeze(logits, -1), 0.0), tf.int64)
    ...
    * Summarizes variables and gradients
    ...
    with tf.name_scope("optimization"):
        ...
        non_embedding_grads_and_vars = [
            (g, v) for (g, v) in zip(grads, tvars) if "embedding" not in v.op.name
        ]
        embedding_grads_and_vars = [
            (g, v) for (g, v) in zip(grads, tvars) if "embedding" in v.op.name
        ]
        ...
        if sync_replicas:
            ...
            tf.add_to_collection(
                tf.GraphKeys.QUEUE_RUNNERS, opt.get_chief_queue_runner()
            )
