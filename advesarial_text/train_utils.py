...
flags.DEFINE_string(
    "train_dir", "/tmp/text_train", "Directory for logs and checkpoints."
)
...
def run_training(
    train_op, loss, global_step, variables_to_restore=None, pretrained_model_dir=None
):
    ...
    with sv.managed_session(
        ...
        if is_chief:
            if pretrained_model_dir:
                maybe_restore_pretrained_model(
                    sess, saver_for_restore, pretrained_model_dir
                )
            if FLAGS.sync_replicas:
...
def maybe_restore_pretrained_model(sess, saver_for_restore, model_dir):
    ...
    if checkpoint_exists:
        tf.logging.info(
            "Checkpoint exists in FLAGS.train_dir; skipping " "pretraining restore"
        )
        return
...
def train_step(sess, train_op, loss, global_step):
    ...
    if global_step_val % 10 == 0:
        ...
        tf.logging.info(
            format_str % (global_step_val, loss_val, examples_per_sec, sec_per_batch)
        )
